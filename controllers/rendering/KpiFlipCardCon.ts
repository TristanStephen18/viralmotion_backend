...
import { updateJson_KpiFlipCards } from "../functions/jsonupdater.ts";
...
export const handleExport = async (req: Request, res: Response) => {
  ...
  console.log("Card color back: ", cardColorBack)

  updateJson_KpiFlipCards(
    backgroundImage,
    title,
    titleFontSize,
    titleFontColor,
  ...
  try {
    ...
    const comp = comps.find((c) => c.id === "KpiFlipCard");
    if (!comp) {
      return res.status(404).json({ error: "Composition not found" });
    }

    // 🗂️ 3. Temporary file paths
    const tmpBaseName = `kpiflipcards-${Date.now()}`;
