...
import { updateJsonConfig_Story } from "../functions/jsonupdater.ts";
...
export const handleExport = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const comp = comps.find((c) => c.id === "StoryTellingVideo");
    if (!comp) {
      return res.status(404).json({ error: "Composition not found" });
    }

    // 🗂️ 3. Temporary file paths
    const tmpBaseName = `storytelling-${Date.now()}`;
